...
async function detectPlayerName() {
    ...
    if (window.alt1) {
        try {
            // Alt1 player detection would go here
    ...
    // If no name detected, show prompt
    if (!AppState.playerName) {
        const name = prompt('Enter your RuneScape username:');
        if (name) {
    ...
    playerNameElements.forEach(el => {
        el.textContent = AppState.playerName || 'Not detected';
    });
    
    // If we have a name, try to load their data
    if (AppState.playerName) {
